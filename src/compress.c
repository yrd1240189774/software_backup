...
BackupResult decompress_file(const char *input_path, const char *output_path, CompressAlgorithm algorithm) {
    ...
    switch (header.algorithm) {
        case COMPRESS_ALGORITHM_HAFF:
            result = huffman_decompress(input_fp, output_fp, header.original_size);
            break;
        case COMPRESS_ALGORITHM_LZ77:
            result = lz77_decompress(input_fp, output_fp, header.original_size);
            break;
        default:
            {
                // 不压缩，直接复制文件
...
BackupResult lz77_decompress(FILE *input_fp, FILE *output_fp, unsigned long original_size) {
    ...
    while (bytes_written < original_size) {
        ...
        if ((token & 0x80) == 0) {
            ...
            // 可能是匹配 (最高位为1)，但也可能是多字节字符的一部分
            // 先尝试作为匹配处理，如果失败则作为字面量处理
            int is_valid_match = 1;
            ...
            if (window_size > 0) {
                ...
                if (bytes_read == 2) {
                    ...
                    // 检查偏移量是否有效
                    if (offset == 0 || offset > window_size || offset > LZ77_WINDOW_SIZE) {
                        is_valid_match = 0;
                        // 偏移量无效，将已读取的2字节偏移量放回文件流
                        fseek(input_fp, -2, SEEK_CUR);
                    }
                } else {
                    is_valid_match = 0;
                    // 读取偏移量失败，将已读取的字节放回文件流（如果有的话）
                    if (bytes_read > 0) {
                        fseek(input_fp, -bytes_read, SEEK_CUR);
                    }
                }
            } else {
                is_valid_match = 0;
            ...
            if (is_valid_match) {
                ...
                if (length > window_size) {
                    length = window_size; // 限制匹配长度不超过窗口大小
                }
                
                // 输出匹配的数据，注意不要超过原始大小
                for (int i = 0; i < length && bytes_written < original_size; i++) {
                    // 确保匹配位置有效
                    if (match_pos >= window_size) {
                        // 匹配位置无效，将token和偏移量作为字面量处理
                        is_valid_match = 0;
                        break;
                    }
