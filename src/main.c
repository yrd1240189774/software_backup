...
// 定时备份全局状态变量
int g_schedule_is_running = 0;          // 0: 未运行, 1: 正在运行
HANDLE g_schedule_thread = NULL;        // 定时备份线程句柄
ScheduledBackupOptions g_current_schedule_options = {0}; // 当前运行的定时备份选项
...
// 初始化中文显示
void init_chinese_display() {
    // 设置控制台输出代码页为GBK，解决中文乱码问题
    // 因为我们的代码是使用GBK编码编译的（Makefile中设置了-fexec-charset=GBK）
    SetConsoleOutputCP(936); // 936是GBK的代码页
    SetConsoleCP(936); // 设置控制台输入代码页也为GBK
}
...
int main(int argc, char *argv[]) {
    // 检查是否为GUI模式
    // 1. 如果有--gui命令行参数，启动GUI
    // 2. 如果没有命令行参数，也启动GUI（方便直接双击运行）
    // 3. 如果有--debug命令行参数，启动GUI并输出调试信息
    int debug_mode = 0;
    if (argc > 1) {
        if (strcmp(argv[1], "--debug") == 0) {
            ...
            // 不是GUI模式，继续执行命令行模式
            goto command_line_mode;
        }
    }
    
    return GUI_Init(debug_mode);
    
command_line_mode:
    
    // 初始化中文显示
    init_chinese_display();
