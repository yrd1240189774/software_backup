    ...
    if (*operation == 0) {
        ...
        while (i < argc) {
            if (strcmp(argv[i], "-s") == 0 && i + 1 < argc) {
                ...
                if (strcmp(argv[i + 1], "mypack") == 0) {
                    backup_opt->pack_algorithm = PACK_ALGORITHM_MYPACK;
                } else if (strcmp(argv[i + 1], "tar") == 0) {
...
int main(int argc, char *argv[]) {
    ...
    switch (operation) {
        ...
        case 4: // 加密
            ...
            result = encrypt_file(encrypt_input, encrypt_output, encrypt_algorithm, encrypt_key);
            if (result == BACKUP_SUCCESS) {
                printf("加密成功！\n");
            } else {
                printf("加密失败，错误码: %d\n", result);
        ...
        case 5: // 解密
            ...
            result = decrypt_file(decrypt_input, decrypt_output, decrypt_algorithm, decrypt_key);
            if (result == BACKUP_SUCCESS) {
                printf("解密成功！\n");
            } else {
                printf("解密失败，错误码: %d\n", result);
